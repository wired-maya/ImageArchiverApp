...
// - figure out how to download the weird gif things
...
namespace ImageArchiverApp
{
    class PixivDownloader
    {
        ...
        private async Task PixivImageDownload(string uri, PixivUserProfile profile, PixivIllustration illust, CancellationToken ct, int i = 0, bool multiplePages = false)
        {
            ...
            string path = Path.Combine(form.FilePath, Tools.RemoveInvalidCharacters(form.PixivOptions["SortByArtist"] ? profile.User.Name : form.PixivOptions["SortByFranchise"] ? "" : ""));
            if (!Directory.Exists(path)) Directory.CreateDirectory(path);
            string fileName = form.PixivOptions["FilesAsTitle"] ? fileName = illust.Title + (multiplePages ? (i + 1).ToString() : "") + uri.Substring(uri.LastIndexOf(".")) : fileName = uri.Substring(uri.LastIndexOf("/") + 1);
